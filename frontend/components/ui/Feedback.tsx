...
export function Skeleton({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "animate-shimmer rounded-2xl bg-[linear-gradient(90deg,var(--color-ink-100)_25%,var(--color-ink-50)_50%,var(--color-ink-100)_75%)] bg-[length:200%_100%]",
        className,
      )}
    />
  );
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-3xl border border-dashed border-ink-200 bg-white/60 px-6 py-12 text-center">
      ...
      {description ? (
        <p className="max-w-sm text-sm leading-7 text-ink-500">{description}</p>
      ) : null}
...
export function ErrorState({
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-3xl border border-coral-300 bg-coral-100/50 px-6 py-10 text-center">
      ...
      <h3 className="text-base font-bold text-ink-900">{title}</h3>
      {description ? (
        <p className="max-w-sm text-sm leading-7 text-ink-600">{description}</p>
      ) : null}
      {action ? <div className="mt-1">{action}</div> : null}
    </div>
  );
